...
(defvar jumblr-precompute-file
  (expand-file-name "jumblr-games.el" user-emacs-directory)
  "Filename to store the precomputed Jumblr games.")
...
;;
(defvar jlr-game-data-list nil
  "List of precomputed word games for Jumblr.")

(defconst jlr-save-file-header
  ";;; Automatically generated by `jlr-save-list' on %s.\n"
  "Header for the precomputed games file `jumblr-precompute-file'.")

(defconst jlr-save-file-coding-system
  (if (coding-system-p 'utf-8-emacs)
      'utf-8-emacs
    'emacs-mule)
  "Coding system of the file `jumblr-precompute-file'.")

(defun jlr-format-dump (var buf)
  "Return a setq statement which recreates the lisp object VAR"
  (pp `(setq ,var (quote ,(symbol-value var)))
      buf))

(defun jlr-save-list ()
  "Save the list of precomputed games.
Write data into the file specified by `jumblr-precompute-file'."
  (interactive)
  (condition-case error
      (let ((buf (get-buffer-create "*Jumblr compute*")))
        (with-current-buffer buf
          ...
          (insert "\n\n;; Local Variables:\n"
                  ...
                  ";; End:\n")
          (write-file (expand-file-name jumblr-precompute-file))
          nil))
    (error
     (warn "jlr-save-list mode: %s" (error-message-string error)))))

(defun jlr-load-list ()
  "Load a previously saved list of precomputed games.
Read data from the file specified by `jumblr-precompute-file'."
  (setq jlr-game-data-list nil)
  (let ((file (expand-file-name jumblr-precompute-file)))
    (when (file-readable-p file)
      (load-file file))))



;;; remake a list of precomputed games
;;
(defun jlr-recompute-games (num min max)
  "Remake the list of precomputed Jumblr games.
Make a list of NUM games, consisting of words between MIN and MAX
characters long (inclusive).  Store this in the variable
`jlr-game-data-list' and save it to the file
`jumblr-precompute-file'.  Note that running this make take a
*long* time (let it run overnight), but it will speed up
subsequent games significantly."
  (setq jlr-game-data-list nil)
  (let ((words (--filter (and (>= max (length it))
                              (<= min (length it))) word-list)))
    (dotimes (i num)
      (let* ((n (random (length words)))
             (word (nth n words))
             (data (jlr-create-game-data word)))
...
(defun jlr-make-new-game-data ()
  ...
  (let* ((words (--filter (and (>= (length it) 5)
         ...
         (word (nth n words)))
    (message "computing a new game... (run jlr-recompute-games to precompute)")
    (setq jlr-game-data (jlr-create-game-data word))
    (message "computing a new game...done."))
  nil)

(defun jlr-pop-precomputed-game ()
  "Select a precomputed Jumblr game and remove it from the list."
  (setq jlr-game-data (car jlr-game-data-list))
  (setq jlr-game-data-list (cdr jlr-game-data-list))
  (jlr-save-list)
  nil)
...
(defun jumblr-new-game ()
  ...
  (unless jlr-game-data-list (jlr-load-list))

  ;; select a precomputed game if available; otherwise, compute a new
  ;; one.
  (if jlr-game-data-list
      (jlr-pop-precomputed-game)
